...
exports.maybeGenerateDailyTip = maybeGenerateDailyTip;
const google_genai_1 = require("@langchain/google-genai");
const prompts_1 = require("@langchain/core/prompts");
const output_parsers_1 = require("@langchain/core/output_parsers");
const db_1 = require("./db");
const parser = new output_parsers_1.StringOutputParser();
const template = prompts_1.PromptTemplate.fromTemplate(`You are a concise study companion. Using the context below, provide one actionable tip (at most two sentences) that helps the user stay focused or take care of their wellbeing.
...
function shouldRefreshTip(lastGeneratedAt) {
    if (!lastGeneratedAt)
        return true;
...
const titleCase = (value) => value.length === 0 ? value : value[0].toUpperCase() + value.slice(1);
const lowerFirst = (value) => value.length === 0 ? value : value[0].toLowerCase() + value.slice(1);
function buildFallbackTip(stats, prefs) {
    const mood = typeof stats.mood === "string" && stats.mood.length > 0
        ? stats.mood.toLowerCase()
        : "steady";
    const focus = Number.isFinite(stats.focus_level) && stats.focus_level >= 0
        ? Number(stats.focus_level)
        : null;
    const temperature = Number.isFinite(stats.room_temperature) && stats.room_temperature !== null
        ? Number(stats.room_temperature)
        : null;
    const noise = Number.isFinite(stats.noise_pollution) && stats.noise_pollution !== null
    ...
    if (focus !== null) {
        if (focus <= 3) {
            suggestions.push("Take a two-minute reset and breathe deeply before the next block");
        }
        else if (focus >= 7) {
            suggestions.push("Capture the next task in your notes so the momentum keeps rolling");
        }
        else {
            suggestions.push("Set a gentle timer for your next focus sprint to stay present");
        }
    }
    if (temperature !== null) {
        if (temperature >= 26) {
...
async function maybeGenerateDailyTip() {
    ...
    const setAndReturnFallback = () => {
        (0, db_1.setDailyTip)(fallback, new Date().toISOString());
        return fallback;
    };
    ...
    try {
        ...
        const tip = (await template.pipe(model).pipe(parser).invoke(context)).trim();
        if (tip.length > 0) {
            (0, db_1.setDailyTip)(tip, new Date().toISOString());
            return tip;
        }
    }
    catch (error) {
